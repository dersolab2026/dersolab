        ...
        <table className="w-full min-w-[520px] text-sm">
          <thead>
            <tr className="border-b-2 border-white/10">
              <th className="py-1.5 pr-2 text-left font-bold text-slate-200">Ders</th>
              <th className="py-1.5 px-1 text-center font-bold text-slate-400">Yanlış</th>
              {TIPLER.map((t) => (
                <th key={t} className="py-1.5 px-1 text-center text-xs font-bold text-slate-200">
                  {ERROR_TYPE_LABELS[t]}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {yanlisiOlanlar.map((s) => {
              ...
              return (
                <tr key={s.name} className="border-b border-white/10/15">
                  <td className="py-1.5 pr-2 font-bold text-slate-200">{s.name}</td>
                  <td className={`py-1.5 px-1 text-center font-bold tabular-nums ${asim ? 'text-red-600' : 'text-slate-400'}`}>
                  ...
                  {TIPLER.map((t) => (
                    <td key={t} className="py-1.5 px-1">
                      <input
                        ...
                        className="w-full min-w-[52px] rounded-lg border border-white/5 bg-white px-1.5 py-1 text-center font-bold text-slate-200"
                      />
                    </td>
                  ))}
                </tr>

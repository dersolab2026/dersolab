  ...
  return (
    <div className={`${PIXEL_CARD} p-5 space-y-5`}>
      <div className="flex items-center gap-2">
        ...
        <p className="font-bold text-slate-200">Deneme Analizi</p>
      </div>
            ...
            return (
              <button
                ...
                className={`rounded-lg border border-white/5 px-2.5 py-1 text-xs font-bold transition-all ${
                  secili ? 'bg-[#6FA89E] text-[#F4F1E8]' : 'bg-white/5 text-slate-400 line-through'
                }`}
              >
                {e.examName}
              </button>
      ...
      {hedefNet !== null && (
        <div className="flex items-start gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-3">
          ...
          <div>
            <p className="text-sm font-bold text-slate-200">{degerlendirme.mesaj}</p>
      ...
      {(() => {
        ...
        return (
          <p className="rounded-xl border border-white/10 bg-[#F6EAD2] px-4 py-3 text-sm font-semibold text-slate-200">
            {uyari}
          </p>
        )
      })()}
          ...
          {dersDurumu.map((d) => {
            ...
            return (
              <div key={d.ad} className="flex items-center gap-3">
                <span className="w-40 shrink-0 truncate text-sm font-bold text-slate-200" title={d.ad}>
                  {d.ad}
                </span>
                ...
                <span className="w-24 shrink-0 text-right text-sm font-bold tabular-nums text-slate-200">
...
function HataTipiDagilimi({ entries }: { entries: ExamResultEntry[] }) {
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex h-7 overflow-hidden rounded-md border border-white/5">
        {TIPLER.map((t) => {
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1">
        {TIPLER.map((t) => (
          <span key={t.anahtar} className="flex items-center gap-1.5 text-xs font-semibold text-slate-200/80">
            <span className="inline-block h-3 w-3 rounded-sm border border-white/10" style={{ backgroundColor: t.renk }} />
      ...
      <p className="text-sm font-bold text-slate-200">{ERROR_TYPE_PRESCRIPTION[baskin.anahtar]}</p>
    </div>
  )
}
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-baseline justify-between">
        <p className="text-sm font-bold text-slate-400">Net gelişimi</p>

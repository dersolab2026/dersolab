...
export function StudentIntakeSummary({ veri }: { veri: IntakeData }) {
  ...
  return (
    <div className={`${PIXEL_CARD} space-y-4 p-5`}>
      ...
      {form?.whoWanted && (
        <div className={`flex items-center gap-2 rounded-xl border-4 border-[var(--cizgi)] px-3 py-2 ${
          form.whoWanted === 'ailem' ? 'bg-[#F6EAD2]' : 'bg-[var(--yuzey-ic)]'
        }`}>
          {form.whoWanted === 'ailem' && <AlertTriangle className="h-4 w-4 shrink-0 text-[#8A5A00]" />}
          <p className="text-sm font-bold text-[var(--yazi)]">{KIM_ETIKET[form.whoWanted]}</p>
        </div>
      )}
      ...
      {satirlar.length > 0 && (
        <div className="space-y-2">
          {satirlar.map((s) => (
            <div key={s.etiket} className="rounded-xl border-2 border-[var(--cizgi)] bg-[var(--yuzey-ic)] p-3">
              <p className="mb-1.5 text-xs font-bold uppercase tracking-wide text-[var(--yazi)]/60">{s.etiket}</p>
              <div className="flex flex-wrap gap-1.5">
                {s.degerler.map((d) => (
                  <span key={d}
                    className="rounded-lg border-2 border-[var(--cizgi)] bg-[var(--yuzey)] px-2 py-0.5 text-sm font-semibold text-[var(--yazi)]">
                    {d}
                  </span>
                ))}
              </div>
      ...
      {form?.notes && (
        <div className="rounded-xl border-2 border-[var(--cizgi)] bg-[var(--yuzey-ic)] p-3">
          <p className="text-xs font-bold uppercase tracking-wide text-[var(--yazi)]/60">Eklemek istediği</p>
          <p className="text-sm font-semibold text-[var(--yazi)] whitespace-pre-wrap">{form.notes}</p>
        </div>
      )}

      {son && (
        <div className="space-y-3 border-t-2 border-[var(--cizgi)]/10 pt-3">
          ...
          {zayif.length > 0 && (
            <p className="text-sm font-bold text-[var(--yazi)]">
              En zayıf iki alan: <span className="text-[var(--vurgu-yazi)]">{zayif.map((z) => z.ad).join(' ve ')}</span>
            </p>
          )}

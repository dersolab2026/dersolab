  ...
  if (logs.length === 0) {
    return (
      <div className={`${PIXEL_CARD} p-5`}>
        <p className="font-bold text-[#1B2430]">Çalışma Günlüğü</p>
  ...
  return (
    <div className={`${PIXEL_CARD} space-y-4 p-5`}>
      <div className="flex items-center gap-2">
        <BookOpen className="h-5 w-5 text-[#1B2430]" />
        <p className="font-bold text-[#1B2430]">Çalışma Günlüğü</p>
      </div>

      <div className="space-y-2">
        <p className="text-sm font-bold text-[#1B2430]/70">Derse göre toplam</p>
        {dersToplamlari.map((d) => (
          <div key={d.subject} className="flex items-center gap-3">
            <span className="w-36 shrink-0 truncate text-sm font-bold text-[#1B2430]" title={d.subject}>
              {d.subject}
            </span>
            <div className="h-5 flex-1 overflow-hidden rounded-md border-2 border-[#1B2430] bg-white">
              <div
      ...
      <div className="space-y-1.5">
        <p className="text-sm font-bold text-[#1B2430]/70">Son kayıtlar</p>
        <div className="max-h-72 space-y-1.5 overflow-y-auto pr-1">
          {logs.slice(0, 40).map((l) => (
            ...
            >
              ...
              {l.topic && <span className="text-[#1B2430]/70">— {l.topic}</span>}
              <span className="ml-auto shrink-0 font-semibold tabular-nums text-[#1B2430]/70">
                {l.hours ? `${l.hours} sa` : ''}

  ...
  if (logs.length === 0) {
    return (
      <div className={`${PIXEL_CARD} p-5`}>
        <p className="font-bold text-slate-200">Çalışma Günlüğü</p>
  ...
  return (
    <div className={`${PIXEL_CARD} space-y-4 p-5`}>
      ...
      <div className="space-y-2">
        <p className="text-sm font-bold text-slate-400">Derse göre toplam</p>
        {dersToplamlari.map((d) => (
          <div key={d.subject} className="flex items-center gap-3">
            <span className="w-36 shrink-0 truncate text-sm font-bold text-slate-200" title={d.subject}>
              {d.subject}
            </span>
            <div className="h-5 flex-1 overflow-hidden rounded-md border border-white/5 bg-white">
              <div
                className="h-full bg-[#6FA89E]"
                style={{ width: `${Math.max(4, Math.round((d.saat / enYuksek) * 100))}%` }}
              />
            </div>
            <span className="w-28 shrink-0 text-right text-sm font-bold tabular-nums text-slate-200">
              {d.saat} sa
              {d.soru > 0 && <span className="text-slate-400"> · {d.soru} soru</span>}
            </span>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-1.5">
        <p className="text-sm font-bold text-slate-400">Son kayıtlar</p>
        <div className="max-h-72 space-y-1.5 overflow-y-auto pr-1">
          {logs.slice(0, 40).map((l) => (
            <div
              key={l.id}
              className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5 rounded-lg border border-white/5 bg-white px-3 py-1.5 text-sm"
            >
              ...
              <span className="font-bold text-slate-200">{l.subject}</span>
              {l.topic && <span className="text-slate-400">— {l.topic}</span>}
              <span className="ml-auto shrink-0 font-semibold tabular-nums text-slate-400">
                {l.hours ? `${l.hours} sa` : ''}

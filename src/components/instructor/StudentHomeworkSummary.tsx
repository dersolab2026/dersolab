...
const DURUM: Record<string, { etiket: string; renk: string; arka: string }> = {
  completed: { etiket: 'Tamamlandı', renk: 'text-[var(--yazi-ters)]', arka: 'bg-[var(--ikincil-zemin)]' },
  submitted: { etiket: 'Teslim edildi', renk: 'text-[var(--yazi)]', arka: 'bg-[#E8C468]' },
  assigned: { etiket: 'Bekliyor', renk: 'text-[var(--yazi)]', arka: 'bg-[var(--yuzey-ic)]' },
}
  ...
  if (items.length === 0) {
    return (
      <div className={`${PIXEL_CARD} p-5`}>
        <p className="font-bold text-[var(--yazi)]">Ödevler</p>
      ...
      <div className="space-y-2">
        {items.map((h) => {
          ...
          return (
            <div
              key={h.id}
              className="flex flex-wrap items-center justify-between gap-2 rounded-xl border-2 border-[var(--cizgi)] bg-[var(--yuzey-ic)] px-3 py-2"
            >
              <div className="flex min-w-0 items-center gap-2">
                {h.status === 'completed' ? (
                  <CheckCircle2 className="h-4 w-4 shrink-0 text-[var(--ikincil-yazi)]" />
                ) : gecikti ? (
                  <AlertTriangle className="h-4 w-4 shrink-0 text-[#C2410C]" />
                ) : (
                  <Clock className="h-4 w-4 shrink-0 text-[var(--yazi)]/50" />
                )}
                <span className="truncate text-sm font-bold text-[var(--yazi)]">{h.title}</span>
              </div>
              <div className="flex shrink-0 items-center gap-2">
                {h.dueDate && (
                  <span className={`text-xs font-semibold ${gecikti ? 'text-[#C2410C]' : 'text-[var(--yazi)]/60'}`}>
                    {new Date(h.dueDate).toLocaleDateString('tr-TR')}
                  </span>
                )}
                {h.teslimSayisi > 0 && (
                  <span className="text-xs font-semibold text-[var(--yazi)]/60">
                    {h.teslimSayisi} dosya
                  </span>
                )}
                <span
                  className={`rounded-lg border-2 border-[var(--cizgi)] px-2 py-0.5 text-xs font-bold ${d.arka} ${d.renk}`}
                >
                  {d.etiket}
                </span>
              </div>

...
export function InstructorCard({ instructor, sessionTypeHint, nextSlot }: InstructorCardProps) {
  ...
  return (
    <Link href={href}>
      <div className="h-full flex flex-col bg-[var(--yuzey)] rounded-2xl p-5 border-4 border-[var(--cizgi)] shadow-[0_6px_0_var(--golge)] transition-all hover:-translate-y-0.5">
        <div className="flex items-center gap-3">
          <Avatar className="h-12 w-12 border-2 border-[var(--cizgi)]">
          ...
          <div>
            <p className="font-bold text-[var(--yazi)]">{instructor.name}</p>
            {instructor.completedLessonCount > 0 && (
              <span className="flex items-center gap-1 text-xs font-bold text-[var(--ikincil-yazi)]">
        ...
        <div className="mt-3 space-y-2 flex-1">
          <div className="flex flex-wrap gap-1">
            {instructor.subjects.map((subject) => (
              <span key={subject} className="px-2 py-0.5 rounded-lg border-2 border-[var(--cizgi)] bg-[var(--yuzey-ic)] text-[var(--yazi)] text-xs font-bold">
                {subject}
              </span>
            ))}
          </div>
          {instructor.bio && <p className="line-clamp-2 text-sm font-semibold text-[var(--yazi)]/70">{instructor.bio}</p>}
          {instructor.introVideoUrl && (
            <span className="flex items-center gap-1 text-xs font-bold text-[var(--yazi)]/70">
        ...
        {nextSlot ? (
          <span className="mt-3 inline-flex w-fit items-center gap-1.5 px-2 py-1 rounded-lg border-2 border-[var(--cizgi)] bg-[var(--ikincil-zemin)] text-[var(--yazi-ters)] text-xs font-bold">
            <CalendarClock className="h-3.5 w-3.5" />
            İlk müsait: {formatNextSlot(nextSlot)}
          </span>
        ) : (
          <span className="mt-3 inline-block w-fit px-2 py-0.5 rounded-lg border-2 border-[var(--cizgi)] bg-[var(--yuzey-ic)] text-[var(--yazi)] text-xs font-bold">
            Yakında müsait
          </span>
        )}
      </div>

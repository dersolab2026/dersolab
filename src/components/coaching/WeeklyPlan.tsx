    ...
    <div className={`${PIXEL_CARD} space-y-4 p-5`}>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <p className="font-bold text-slate-200">Haftalık Plan</p>
        </div>
      ...
      {items.length > 0 && (
        <div className="flex items-center gap-3">
          <div className="h-6 flex-1 overflow-hidden rounded-md border border-white/5 bg-white">
            <div
          ...
          <span className="shrink-0 text-sm font-black tabular-nums text-slate-200">
            %{tutturma.yuzde}
          </span>
        </div>
      )}
              ...
              <div key={g}>
                ...
                <div className="space-y-1.5">
                  {gunun.map((it) => {
                    ...
                    return (
                      <div key={it.id}
                        className={`flex flex-wrap items-center gap-2 rounded-xl border border-white/5 px-3 py-2 ${
                      ...
                      >
                        <div className="min-w-0 flex-1">
                          <p className="text-sm font-bold text-slate-200">
                            {it.planTime && <span className="mr-1.5 tabular-nums text-slate-400">{it.planTime.slice(0, 5)}</span>}
                            {it.subject}
                            {it.topic && <span className="font-semibold text-slate-400"> — {it.topic}</span>}
                        ...
                        {canEdit && (
                          <div className="flex shrink-0 items-center gap-1">
                            <button type="button" onClick={() => durum(it.id, it.status === 'done' ? 'planned' : 'done')}
                              ...
                              className={`rounded-lg border border-white/5 p-1 ${it.status === 'done' ? 'bg-[#6FA89E] text-white' : 'bg-white/5 text-slate-200'}`}>
                              <Check className="h-3.5 w-3.5" />
                            </button>
                            <button type="button" onClick={() => durum(it.id, it.status === 'skipped' ? 'planned' : 'skipped')}
                              aria-label="Atlandı işaretle" title="Atlandı"
                              className={`rounded-lg border border-white/5 p-1 ${it.status === 'skipped' ? 'bg-[#1B2430] text-white' : 'bg-white/5 text-slate-200'}`}>
      ...
      {acik && canEdit && (
        <div className="space-y-3 rounded-xl border border-white/10 bg-white p-4">
          <div className="grid gap-3 sm:grid-cols-2">
            <div>
              <label className="mb-1 block text-sm font-bold text-slate-200">Gün</label>

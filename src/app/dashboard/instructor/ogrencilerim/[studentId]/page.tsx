    ...
    >
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <div className={`${PIXEL_CARD} p-4`}>
          <p className="text-xs font-bold uppercase tracking-wide text-[var(--yazi)]/60">Kayıtlı deneme</p>
          <p className="text-2xl font-black text-[var(--yazi)]">{veri.exams.length}</p>
        ...
        <div className={`${PIXEL_CARD} p-4`}>
          <p className="text-xs font-bold uppercase tracking-wide text-[var(--yazi)]/60">Ödev</p>
          <p className="text-2xl font-black text-[var(--yazi)]">
            {veri.homework.filter((h) => h.status === 'completed').length}
            <span className="text-base text-[var(--yazi)]/50">/{veri.homework.length}</span>
          </p>
          <p className="text-xs font-semibold text-[var(--yazi)]/70">tamamlanan</p>
        </div>
        <div className={`${PIXEL_CARD} p-4`}>
          <p className="text-xs font-bold uppercase tracking-wide text-[var(--yazi)]/60">Kayıtlı çalışma</p>
          <p className="text-2xl font-black text-[var(--yazi)]">
            {veri.dersToplamlari.reduce((t, d) => t + d.saat, 0).toFixed(1)}
            <span className="text-base text-[var(--yazi)]/50"> sa</span>
          </p>
          <p className="text-xs font-semibold text-[var(--yazi)]/70">
      ...
      {veri.exams.length === 0 ? (
        <p className={`${PIXEL_CARD} p-4 font-semibold text-[var(--yazi)]`}>

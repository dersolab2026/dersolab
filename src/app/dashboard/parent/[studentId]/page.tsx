...
function Rozet({ metin, vurgu }: { metin: string; vurgu?: boolean }) {
  return (
    <span
      className={`inline-block rounded-lg border border-white/5 px-2 py-0.5 text-xs font-bold ${
        vurgu ? 'bg-[#6FA89E] text-[#F4F1E8]' : 'bg-white/5 text-slate-200'
      }`}
    >
      {metin}
    </span>
...
function Bolum({ baslik, bos, children }: { baslik: string; bos: string; children: React.ReactNode }) {
  return (
    <div className={`${PIXEL_CARD} space-y-3 p-5`}>
      <p className="font-bold text-slate-200">{baslik}</p>
      {children ?? <p className="text-sm font-semibold text-slate-400">{bos}</p>}
    </div>
  )
}
...
export default async function ParentStudentDetailPage({
  ...
  return (
    ...
    >
      <Link href="/dashboard/parent" className="text-sm font-bold text-slate-400 hover:underline">
        &larr; Öğrencilerim
      </Link>
      ...
      <Bolum baslik="Yaklaşan Dersler" bos="Planlanmış ders yok.">
        {yaklasan.length > 0 ? (
          <ul className="space-y-2">
            {yaklasan.map((b) => (
              <li key={b.id} className="flex flex-wrap items-center justify-between gap-2 border-b border-white/10 pb-2 last:border-0">
      ...
      <Bolum baslik="Bekleyen Ödevler" bos="Bekleyen ödev yok.">
        {bekleyenOdev.length > 0 ? (
          <ul className="space-y-2">
            {bekleyenOdev.map((h) => (
              <li key={h.id} className="flex flex-wrap items-center justify-between gap-2 border-b border-white/10 pb-2 last:border-0">
                <span className="text-sm font-semibold text-slate-200">
                  {h.title}
                  {h.dueDate && (
            ...
            <table className="w-full min-w-[420px] text-sm">
              <thead>
                <tr className="border-b-2 border-white/10 text-left">
                  <th className="py-1.5 pr-2 font-bold text-slate-200">Deneme</th>
                  <th className="py-1.5 px-2 font-bold text-slate-200">Tür</th>
                  <th className="py-1.5 px-2 font-bold text-slate-200">Tarih</th>
                  <th className="py-1.5 pl-2 text-right font-bold text-slate-200">Net</th>
                </tr>
              </thead>
              <tbody>
                {veri.examResults.slice(0, 10).map((e) => (
                  <tr key={e.id} className="border-b border-white/10/15">
                    <td className="py-1.5 pr-2 font-semibold text-slate-200">{e.examName}</td>
                    <td className="py-1.5 px-2 font-semibold text-slate-400">{EXAM_TYPE_LABELS[e.examType]}</td>
                    <td className="py-1.5 px-2 font-semibold text-slate-400">
                      {new Date(e.examDate).toLocaleDateString('tr-TR', { dateStyle: 'medium' })}
                    </td>
                    <td className="py-1.5 pl-2 text-right font-bold tabular-nums text-slate-200">
                      {e.net.toFixed(2)}
                    </td>
                  </tr>
                ))}
        ...
        {veri.purchases.length > 0 ? (
          <ul className="space-y-2">
            {veri.purchases.map((p) => (
              <li key={p.id} className="flex flex-wrap items-center justify-between gap-2 border-b border-white/10 pb-2 last:border-0">
                <span className="text-sm font-semibold text-slate-200">
                  {kisaTarihSaat(p.createdAt)} · {p.creditsGranted} kredi
                  {p.amountPaid !== null && (
                    <span className="text-slate-400"> · {p.amountPaid.toLocaleString('tr-TR')} ₺</span>
                  )}
                </span>

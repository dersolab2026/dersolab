  ...
  try {
    await resend.emails.send({
      ...
      subject: 'Takvimini Bağlamayı Unutma - DersoLab',
        html: buildEmailHtml('Takvimini Bağlamayı Unutma', `<p>Merhaba ${params.name},</p>
  ...
  try {
    await resend.emails.send({
      ...
      subject: 'Hoş Geldin Paketini Kaçırma - DersoLab',
        html: buildEmailHtml('Hoş Geldin Paketini Kaçırma', `<p>Merhaba ${params.name},</p>
        <p>DersoLab'da henüz kullanmadığın bir Hoş Geldin Paketin var: kredi harcamayan ücretsiz bir tanışma dersi.</p>
        <p><a href="${appUrl}/demo-ders">Hemen talep et</a>, uygun bir eğitmen seninle iletişime geçsin.</p>`),
    })
  } catch (err) {
